...
export default function ReactViewBase({viewId, children, className = '', handleChange, ...props}: TransitionProps) {

    const target = useRef<HTMLDivElement>(null)
    const [targetClass, setTargetClass] = useState<string>(ViewsProxy.GetView(viewId).viewState)
    ...
    useEffect(() => {
        ...
        const onViewStateChange = (view: SuperViewBase, viewState: ViewState): void => {
    ...
    return (
        <div {...props} id={viewId} className={`view h-dvh w-screen ${className} ${targetClass}`} ref={target} key={viewId}>
            {children}
        </div>
    )
}